...
class CompactCalendarController {
    ...
    private int calenderHeaderColor;
    private int calenderHolidayTxtColor;
    private int calenderHolidayTxtColorOther;
    private int calenderCurrentDateColor;
    ...
    CompactCalendarController(Paint dayPaint, OverScroller scroller, Rect textSizeRect, AttributeSet attrs,
                              ...
                              Locale locale, TimeZone timeZone, HolidayContainer holidayContainer,int calenderCurrentDateColor) {
        ...
        this.calenderHeaderColor = calenderTextColor;
        this.holidayContainer = holidayContainer;
        this.calenderHolidayTxtColor = calenderTextColor;
        this.calenderHolidayTxtColorOther = calenderTextColor;
        this.calenderCurrentDateColor=calenderCurrentDateColor;
    ...
    private void loadAttributes(AttributeSet attrs, Context context) {
        if (attrs != null && context != null) {
            ...
            try {
                ...
                currentSelectedDayTextColor = typedArray.getColor(R.styleable.CompactCalendarView_compactCalendarCurrentSelectedDayTextColor, currentSelectedDayBackgroundColor);
                ...
                calenderHeaderColor = typedArray.getColor(R.styleable.CompactCalendarView_compactCalenderHeaderColor, calenderHeaderColor);
                calenderHolidayTxtColor = typedArray.getColor(R.styleable.CompactCalendarView_compactCalenderHolidayTextColor, calenderHolidayTxtColor);
                calenderHolidayTxtColorOther = typedArray.getColor(R.styleable.CompactCalendarView_compactCalenderHolidayTextColorOther, calenderHolidayTxtColorOther);
                 calenderCurrentDateColor = typedArray.getColor(R.styleable.CompactCalendarView_compactCurrentDateColor, calenderCurrentDateColor);
            } finally {
                typedArray.recycle();
            }
        }
...
Log.i("event_adding"," event name"+eventsList.get(0).getEventName());
dayPaint.setColor(eventsList.get(0).getColor());
                            canvas.drawText(eventsList.get(0).getEventName(), xPosition, yPosition, dayPaint);
//                        }
//                    }
                }
            }
    ...
    void drawMonth(Canvas canvas, Calendar monthToDrawCalender, int offset) {
        ...
        int dayColor = calenderTextColor;
        List<Holidays> uniqHolidays = holidayContainer.getHolidayForMonthAndYear(currentMonth, monthToDrawCalender.get(Calendar.YEAR));
        ...
        for (int dayColumn = 0, dayRow = 0; dayColumn <= 6; dayRow++) {
            ...
            if (dayRow == 0) {
                ...
                if (shouldDrawDaysHeader) {
                    dayPaint.setColor(calenderTextColor);
//                    dayPaint.setTypeface(Typeface.DEFAULT_BOLD); changed by me
                    dayPaint.setStyle(Paint.Style.FILL);
//                    dayPaint.setColor(calenderTextColor);
                    dayPaint.setColor(calenderHeaderColor);
                ...
                if (day <= 0) {
                    if (displayOtherMonthDays) {
                        ...
                        int txtColor;
                        if (dayColumn != 0 && dayColumn % 6 == 0) {
                            txtColor = calenderHolidayTxtColorOther;
                        } else {
                            txtColor = otherMonthDaysTextColor;
                        }

                        dayPaint.setStyle(Paint.Style.FILL);
                        dayPaint.setColor(txtColor);
...
//                    dayColor = defaultCalenderTextColorToUse;

                    if (dayColumn != 0 && dayColumn % 6 == 0) {
                        dayColor = calenderHolidayTxtColor;
                    } else {
                        dayColor = defaultCalenderTextColorToUse;
                    }

                    if (uniqHolidays != null) {
                        for (int i = 0; i < uniqHolidays.size(); i++) {
                            Calendar calendar = Calendar.getInstance();
                            calendar.setTimeInMillis(uniqHolidays.get(i).getTimeInMillis());
                   ...
                   if (currentCalender.get(Calendar.DAY_OF_MONTH) == day && isSameMonthAsCurrentCalendar && !isAnimatingWithExpose) {
                   dayPaint.setColor(calenderCurrentDateColor);
                } else if (isSameYearAsToday && isSameMonthAsToday && todayDayOfMonth == day && !isAnimatingWithExpose) {
                    // TODO calculate position of circle in a more reliable way
                   dayPaint.setColor(calenderCurrentDateColor);
                   }else{
...
//        drawHolidays(canvas, monthToDrawCalender, offset);
    }


//    void drawHolidays(Canvas canvas, Calendar currentMonthToDrawCalender, int offset) {
//        int currentMonth = currentMonthToDrawCalender.get(Calendar.MONTH);
//        List<Holidays> uniqEvents = holidayContainer.getHolidayForMonthAndYear(currentMonth, currentMonthToDrawCalender.get(Calendar.YEAR));
//
//        boolean shouldDrawCurrentDayCircle = currentMonth == todayCalender.get(Calendar.MONTH);
//        boolean shouldDrawSelectedDayCircle = currentMonth == currentCalender.get(Calendar.MONTH);
//
...
//        if (uniqEvents != null) {
//            for (int i = 0; i < uniqEvents.size(); i++) {
//                Holidays events = uniqEvents.get(i);
//                long timeMillis = events.getTimeInMillis();
//                eventsCalendar.setTimeInMillis(timeMillis);
//
//                int dayOfWeek = getDayOfWeek(eventsCalendar);
...
//                }
//
//                List<Holiday> eventsList = events.getHolidays();
//                int dayOfMonth = eventsCalendar.get(Calendar.DAY_OF_MONTH);
//                int eventYear = eventsCalendar.get(Calendar.YEAR);
//                boolean isSameDayAsCurrentDay = shouldDrawCurrentDayCircle && (todayDayOfMonth == dayOfMonth) && (eventYear == currentYear);
//                boolean isCurrentSelectedDay = shouldDrawSelectedDayCircle && (selectedDayOfMonth == dayOfMonth);
...
//                if (shouldDrawIndicatorsBelowSelectedDays || (!shouldDrawIndicatorsBelowSelectedDays && !isSameDayAsCurrentDay && !isCurrentSelectedDay) || animationStatus == EXPOSE_CALENDAR_ANIMATION) {
////                    if (eventIndicatorStyle == FILL_LARGE_INDICATOR || eventIndicatorStyle == NO_FILL_LARGE_INDICATOR) {
////                        Holiday event = eventsList.get(0);
////                        drawEventIndicatorCircle(canvas, xPosition, yPosition, event.getColor());
////                    } else {
////                        yPosition += indicatorOffset;
////                        // offset event indicators to draw below selected day indicators
...
//                    canvas.drawText(String.valueOf(12), xPosition, yPosition, dayPaint);
//
//                }
//            }
//        }
//    }
    ...
    void addHoliday(Holiday holiday) {
        holidayContainer.addEvent(holiday);
    }
}
